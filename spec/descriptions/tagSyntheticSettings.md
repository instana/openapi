...
- **configuration** An object which has two properties: syntheticType and the corresponding configuration object:
    ...
    - **timeout** The timeout to be used by the PoP playback engines running the test. Values in integer followed by time unit (ms, s, m). If timeout is not provided the playback engine will use its own timeout value. The default timeout is **1m** in PoP playback engines for HTTPAction and HTTPScript. If user defined timeout value exceeds the `maxTimeout` configured in PoP deployment or `testFrequency` in test configuration, the timeout value does not take effect and PoP playback engines uses the smaller one of `maxTimeout` and `testFrequency` as the actual timeout value.
