...
- **to:** The end of the requested timeframe as a Unix timestamp. The current service time is used as a default if not provided. Please note that it is recommended to use a `to` timestamp in the past for fetching historical events due to eventual consistency. It can take up to **2 minutes** until an event update is reflected in the results.
